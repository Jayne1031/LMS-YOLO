...
def box_xywh_to_xyxy(box):
    """
    将 (x_center, y_center, width, height) 转为 (x1, y1, x2, y2).
    box 的形状为 (..., 4).
    """
...
def compute_iou(pred_xyxy, gt_xyxy, eps=1e-7):
    """
    计算 IoU:
    pred_xyxy 和 gt_xyxy 的形状均为 (N, 4) 或者 (..., 4)，最后维度是 [x1, y1, x2, y2]。
    返回形状 (..., ) 的 IoU。
    """
    # 交集部分
...
def asiou_loss(pred_boxes, gt_boxes, lam=1.0, S0=32.0, eps=1e-7):
    ...
    pred_boxes : Tensor, (N, 4) or (B, N, 4)
        预测框，格式为 (x_center, y_center, w, h).
    gt_boxes   : Tensor, (N, 4) or (B, N, 4)
        真值框，格式为 (x_center, y_center, w, h).
    lam        : float
        ASIoU 中的超参 λ (lambda)，用于调节 α(S) 函数的陡峭程度。
    S0         : float
        尺度阈值 S0，用于确定小目标和大目标的区分位置。
    eps        : float
        避免除零的微小数。
    ...
    -------
    loss : Tensor
        标量或与 pred_boxes 同维度的损失值。
    """

    # 1) 将 (x, y, w, h) 转成 (x1, y1, x2, y2) 用于计算 IoU
    ...
    # 3) 计算中心点距离项
    #    预测框中心 (b_x, b_y)，真实框中心 (b_x^gt, b_y^gt)
    bx, by = pred_boxes[..., 0], pred_boxes[..., 1]
    bx_gt, by_gt = gt_boxes[..., 0], gt_boxes[..., 1]
    center_dist_sq = (bx - bx_gt).pow(2) + (by - by_gt).pow(2)  # ρ^2(b, b^gt)
    ...
    width_dist_sq = (w_pred - w_gt).pow(2)  # ρ^2(w, w^gt)
    height_dist_sq = (h_pred - h_gt).pow(2)  # ρ^2(h, h^gt)

    # 5) 计算目标真实框的尺度 S 及 α(S)
    #    S = sqrt(w^gt * h^gt)
    ...
    #    其中 center_term = center_dist_sq / ((w^c)^2 + (h^c)^2 + eps)
...
def focal_asiou_loss(pred_boxes, gt_boxes, gamma=2.0, lam=1.0, S0=32.0, eps=1e-7):
    ...
    # 2) 计算 ASIoU loss （不含 IoU 相关系数的写法）
    bx, by = pred_boxes[..., 0], pred_boxes[..., 1]
    bx_gt, by_gt = gt_boxes[..., 0], gt_boxes[..., 1]
...
def asiou_loss_improved(
    ...
    """
    改进后的 ASIoU 损失函数（含 Focal 机制）。

    假设 pred_boxes 和 target_boxes 使用 (cx, cy, w, h) 格式：
      - cx, cy: 框中心点坐标
      - w, h:   框宽和高
    与 YOLOv8 中流行的中心点参数化更接近，可减少额外坐标转换。

    参数:
    --------
    pred_boxes (Tensor): 预测框 (N, 4)，分别为 (cx, cy, w, h)
    target_boxes (Tensor): 真实框 (N, 4)，分别为 (cx, cy, w, h)
    lambda_ (float):      α(S) = 1 / (1 + exp(lambda_ * (S - s0))) 的调节参数
    s0 (float):           控制小目标/大目标分界的尺度阈值
    gamma (float):        Focal-ASIoU 中 (1 - IoU)^gamma 的幂次
    eps (float):          避免数值溢出

    返回:
    --------
    loss (Tensor): 标量损失值 (标量)
    """
    assert pred_boxes.shape == target_boxes.shape and pred_boxes.dim() == 2, \
        "输入形状应为 (N, 4)，且 pred_boxes 与 target_boxes 大小一致"
    ...
    # ------------- 2. 计算中心点距离 (DIoU 形式) -------------
    center_dist = (px - gx) ** 2 + (py - gy) ** 2  # 欧氏距离平方

    # 最小外接矩形（包围框）的对角线距离 c^2
    enc_x1 = torch.min(px1, gx1)
    enc_y1 = torch.min(py1, gy1)
    enc_x2 = torch.max(px2, gx2)
    enc_y2 = torch.max(py2, gy2)
    c2 = (enc_x2 - enc_x1) ** 2 + (enc_y2 - enc_y1) ** 2 + eps  # 避免除 0
    ...
    # ------------- 3. 宽高差异 (EIoU 形式) -------------
    # 使用预测框和真实框宽高的最大值来归一化
    cw = torch.max(pw, gw) + eps  # 最大宽
    ch = torch.max(ph, gh) + eps  # 最大高
    ...
    # ------------- 5. 拼装 ASIoU 损失 -------------
    # 公式: L_{ASIoU} = 1 - IoU + center_dist_norm + α(S)*(w_diff_norm + h_diff_norm)
    L_ASIoU = (1.0 - iou) + center_dist_norm + alpha_s * (w_diff_norm + h_diff_norm)

    # ------------- 6. Focal-ASIoU (改为 (1 - IoU)^gamma) -------------
