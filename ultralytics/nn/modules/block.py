...
class C2f(nn.Module):
    ...
    def forward_split(self, x):
        ...
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
from ultralytics.nn.Add import  MGAM, EMLSAttention
class C2f_MGAM(nn.Module):
    ...
    def forward_split(self, x):
        ...
        out = self.cv2(torch.cat(y, 1))
        return self.mgam(out)
class C2f_EMLSAttention(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        self.emlsattention = EMLSAttention(c2)
...
class RepNCSPELAN4_MGAM(nn.Module):
    """CSP-ELAN."""
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).chunk(2, 1))
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3])
        return self.mgam(self.cv4(torch.cat(y, 1)))
    ...
    def forward_split(self, x):
        ...
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
        return self.mgam(self.cv4(torch.cat(y, 1)))
class RepNCSPELAN4_EMLSAttention(nn.Module):
    """CSP-ELAN."""
    ...
    def __init__(self, c1, c2, c3, c4, n=1):
        ...
        self.cv3 = nn.Sequential(RepCSP(c4, c4, n), Conv(c4, c4, 3, 1))
        self.cv4 = Conv(c3 + (2 * c4), c2, 1, 1)
        self.emlsa = EMLSAttention(c2)

    def forward(self, x):
        """Forward pass through RepNCSPELAN4 layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3])
        return self.emlsa(self.cv4(torch.cat(y, 1)))
...
class C3k2_MGAM(C2f):
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.mgam(self.cv2(torch.cat(y, 1)))
...
class C3k2_EMLSAttention(C2f):
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.emlsa(self.cv2(torch.cat(y, 1)))
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, attn_ratio=0.5, use_lca=False):
        ...
        if use_lca:
            from ultralytics.nn.Add.LCA3 import LCA_layers
            self.sc_attn = LCA_layers(dim)
    def forward(self, x):
        """
...
class PSABlock(nn.Module):
    ...
    def __init__(self, c, attn_ratio=0.5, num_heads=4, shortcut=True) -> None:
        ...
        self.attn = Attention(c, attn_ratio=attn_ratio, num_heads=num_heads, use_lca=False)
...
class ConvBNReLU(nn.Sequential):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, groups=1, act=True, enabled=True, warehouse_manager=None, warehouse_name=None):
        ...
        conv = []
        padding = (kernel_size - 1) // 2
        if warehouse_manager is not None:
            # warehouse_manager = Warehouse_Manager(0.0625, 1, 1,
            #                                       1, sharing_range=('layer', 'conv'))
            conv.append(warehouse_manager.reserve(in_planes, out_planes, kernel_size, stride, padding=padding,
                                                  groups=groups, bias=False, warehouse_name=warehouse_name,
                                                  enabled=enabled))
        else:
            conv.append(nn.Conv2d(in_planes, out_planes, kernel_size, stride, padding, groups=groups))


        conv.append(nn.BatchNorm2d(out_planes))
        if act:
...
class LCA(nn.Module):
    ...
    def forward(self, x):
        ...
        out = u * attn
        return out
class UniversalInverted(nn.Module):
    def __init__(self, in_channels, out_channels, start_dw_kernel_size, middle_dw_kernel_size, middle_dw_downsample,
                 stride, expand_ratio, use_lca):
        """An inverted bottleneck block with optional depthwises.
        Referenced from here https://github.com/tensorflow/models/blob/master/official/vision/modeling/layers/nn_blocks.py
        """
        super(UniversalInverted, self).__init__()
        # starting depthwise conv
        # self.warehouse_manager = Warehouse_Manager(0.0625, 1, 1,
        #                                       1, sharing_range=('layer', 'pwconv','dwconv'))
        self.warehouse_manager= None
        self.start_dw_kernel_size = start_dw_kernel_size
        if self.start_dw_kernel_size:
            stride_ = stride if not middle_dw_downsample else 1
            self._start_dw_ = ConvBNReLU(in_channels, in_channels, kernel_size=start_dw_kernel_size, stride=stride_, groups=in_channels, act=False, warehouse_manager=self.warehouse_manager,
                                         warehouse_name='stage{}_layer{}_dwconv{}'.format(0, 0, 0))
        # expansion with 1x1 convs
        expand_filters = make_divisible(in_channels * expand_ratio, 8)
        # expand_filters = out_channels
        self._expand_conv = ConvBNReLU(in_channels, expand_filters, kernel_size=1, warehouse_manager=self.warehouse_manager,warehouse_name='stage{}_layer{}_conv{}'.format(1, 1, 0))
        # middle depthwise conv
        self.middle_dw_kernel_size = middle_dw_kernel_size
        if self.middle_dw_kernel_size:
            stride_ = stride if middle_dw_downsample else 1
            self._middle_dw = ConvBNReLU(expand_filters, expand_filters, kernel_size=middle_dw_kernel_size, stride=stride_, groups=expand_filters, warehouse_manager=self.warehouse_manager,
                                         warehouse_name='stage{}_layer{}_pwconv{}'.format(1, 2, 0))
        # projection with 1x1 convs
        self._proj_conv = ConvBNReLU(expand_filters, out_channels, kernel_size=1, stride=1, act=False, warehouse_manager=self.warehouse_manager,warehouse_name='stage{}_layer{}_conv{}'.format(1, 3, 0))
        self.use_lca = use_lca
        if self.use_lca:
            self.lca = LCA(out_channels)
